...
begin
  ...
  try
    ...
    I := 1;  // Começamos da linha 1, já que a linha 0 é para cabeçalhos   //Isso e BASICO DO FOR
    while not Q.Eof do
    begin
...
Configuração da StringGrid:

Configuramos o número de colunas (ColCount := 12) e linhas (RowCount := 5 para início).
Preenchemos as células da linha 0 com os cabeçalhos.
...
Para cada registro:
for J := 0 to StringGrid1.ColCount - 1: Este for percorre cada campo (coluna) do registro atual, preenchendo as células da StringGrid.
A função Q.Fields[J].AsString obtém o valor de cada campo e o converte em string para preencher a célula.
Após preencher as colunas de uma linha, chamamos Q.Next para passar para o próximo registro no banco de dados.
O contador I é incrementado após cada registro para preencher a próxima linha da StringGrid.
Ajuste Dinâmico de Linhas:

StringGrid1.RowCount := Q.RecordCount + 1 ajusta o número de linhas na StringGrid com base no número de registros retornados pela consulta, acrescentando 1 para a linha dos cabeçalhos.
O que vai acontecer:
A StringGrid vai exibir todos os registros da tabela gastos_essenciais.
Cada linha representará um registro da tabela.
Cada coluna exibirá um campo do registro, como Id, Produto, Gasto, etc.}